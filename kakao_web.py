...
delay 0.3
tell application "System Events"
  -- 검색창 닫기 (Esc) - 혹시 검색창이 열려있으면 닫기
  key code 53
  delay 0.3
  -- 메시지 입력창으로 포커스 이동 (Esc 후 자동으로 입력창에 포커스)
  key code 53
  delay 0.3
  -- 붙여넣기 (Cmd+V)
  keystroke "v" using command down
...
def send_message(name: str, message: str) -> bool:
    """카카오톡 메시지 전송 (이미지 인식 없이 AppleScript만 사용)"""
    try:
        # 1. 카카오톡 활성화
        ...
        pyperclip.copy(name)
        log(f"   -> 📋 '{name}' 검색 중...")
        run_applescript(SCRIPT_SEARCH_AND_OPEN)
        ...
        run_applescript(SCRIPT_SEND_MESSAGE)
        time.sleep(0.5)
        log(f"   -> ✅ 전송 완료!")
        return True
